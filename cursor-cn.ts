...
const GONG_ZUO_TAI_HTML_MING = "workbench.html";
...
function AnZhuang_MuLu_YouXiao(root: string) {
  ...
  if (!fs.existsSync(path.join(HuoQu_Workbench_MuLu_LuJing(root), "workbench.html"))) return false;
  return Shi_Windows() ? fs.existsSync(path.join(root, "Cursor.exe")) : Shi_MacOS() ? root.endsWith(".app") && fs.existsSync(path.join(root, "Contents", "MacOS")) : true;
}
...
async function TiShi_ShouDong_AnZhuang_LuJing(): Promise<string> {
  ...
  return new Promise(resolve => rl.question("[提示] 未找到 Cursor，请输入安装目录（直接回车取消）：\n> ", (answer: string) => { rl.close(); resolve(answer.trim()); }));
}
async function JianCe_Cursor_LuJing(): Promise<CursorPaths> {
  ...
  candidates.push(path.dirname(SCRIPT_DIR));
  for (const candidate of candidates) {
    if (candidate && AnZhuang_MuLu_YouXiao(candidate)) {
      const installDir = path.resolve(candidate);
      const dataDir = process.env.CURSOR_USER_DATA_DIR || config.dataDir || HuoQu_ShuJu_LuJing();
      BaoCun_LuJing_PeiZhi(installDir, path.resolve(dataDir));
      return { installDir, dataDir: path.resolve(dataDir), valid: true };
    }
  }
  const manual = await TiShi_ShouDong_AnZhuang_LuJing();
  if (manual && AnZhuang_MuLu_YouXiao(manual)) {
    const installDir = path.resolve(manual);
  ...
  return { installDir: path.resolve(configured || env || path.dirname(SCRIPT_DIR)), dataDir: HuoQu_ShuJu_LuJing(), valid: false };
}
...
function HuoQu_Cursor_CLI_LuJing(root = CURSOR_AN_ZHUANG_LU_JING) {
  ...
  return fs.existsSync(p) ? p : HuoQu_Cursor_KeZhiXing_LuJing(root);
}
function GuanBi_Cursor() {
  if (Shi_Windows()) spawnSync("taskkill", ["/IM", "Cursor.exe", "/T", "/F"], { stdio: "ignore" });
  else if (Shi_MacOS()) spawnSync("osascript", ["-e", 'tell application "Cursor" to quit'], { stdio: "ignore" });
  else spawnSync("pkill", ["-f", "[C]ursor"], { stdio: "ignore" });
}
function QiDong_Cursor() {
  const exe = HuoQu_Cursor_KeZhiXing_LuJing();
  const args = [`--user-data-dir=${CURSOR_SHU_JU_LU_JING}`];
  if (Shi_MacOS()) spawnSync("open", ["-na", CURSOR_AN_ZHUANG_LU_JING, "--args", ...args], { stdio: "ignore" });
  else spawn(exe, args, { detached: true, stdio: "ignore", windowsHide: true }).unref();
}
...
function HuoQu_CiDian_WenJian_LuJing(name: string, dir = LOCALIZATION_DIR) { const newer = path.join(dir, name); return fs.existsSync(newer) ? newer : path.join(SCRIPT_DIR, name); }
function HuoQu_CiDian_XianDui_LuJing(file: string) { return path.relative(SCRIPT_DIR, file).replaceAll(path.sep, "/"); }
...
function DuQu_GuangGao_TanChuang_CiDian(): Record<string, string> {
  ...
  try {
    ...
    if (Array.isArray(entries)) {
      for (const x of entries) if (Array.isArray(x) && x.length >= 2 && x[0] && x[1]) out[String(x[0])] = String(x[1]);
    } else if (entries && typeof entries === "object") {
      for (const [k, v] of Object.entries(entries)) if (k && v) out[k] = String(v);
    }
    return out;
...
function HuoQu_HTML_LuJing() { return path.join(HuoQu_GongZuoTai_LuJing(), GONG_ZUO_TAI_HTML_MING); }
...
function JianCha_YiZhuRu() { return fs.existsSync(HuoQu_HTML_LuJing()) && (() => { const text = fs.readFileSync(HuoQu_HTML_LuJing(), "utf8"); return text.includes(INJECTION_MARKER_HTML) || text.includes(LEGACY_INJECTION_MARKER_HTML); })(); }
...
function ShengJi_HTML_ZhuRu_If_Needed() { const file = HuoQu_HTML_LuJing(); const [text, nl] = DuQu_WenBen_BaoLiu_HuanHang(file); const out = text.replaceAll(LEGACY_INJECTION_MARKER_HTML, INJECTION_MARKER_HTML).replaceAll(`./${LEGACY_LOCALIZATION_JS_FILE}`, `./${LOCALIZATION_JS_FILE}`); if (out !== text) { XieRu_WenBen_BaoLiu_HuanHang(file, out, nl); GengXin_JiaoYan_Zhi(); } }
function ShanChu_SuoYou_ZhuRu_JS() { for (const name of [LOCALIZATION_JS_FILE, LEGACY_LOCALIZATION_JS_FILE]) { const file = path.join(HuoQu_GongZuoTai_LuJing(), name); if (fs.existsSync(file)) fs.rmSync(file); } }
...
function YanZheng_VSIX(file: string) { try { const data = fs.readFileSync(file); for (const entry of ZipEntries(data)) ZipRead(data, entry.name); return true; } catch { return false; } }
...
async function XiaZai_ShiChang_VSIX(version: string, target: string) { const r = await fetch(LANGUAGE_PACK_MARKETPLACE_DOWNLOAD.replace("{version}", encodeURIComponent(version)), { headers: { "User-Agent": "Cursor-Localization-Tool", Accept: "application/octet-stream" } }); if (!r.ok) throw new Error(`HTTP ${r.status}`); let data = Buffer.from(await r.arrayBuffer()); if (data[0] === 0x1f && data[1] === 0x8b) data = zlib.gunzipSync(data); ZipRead(data, "extension/package.json"); const temp = target + ".download"; fs.writeFileSync(temp, data); if (!YanZheng_VSIX(temp)) { fs.rmSync(temp, { force: true }); throw new Error("下载的 VSIX 压缩包校验失败"); } fs.renameSync(temp, target); }
...
function HuoQu_Bundled_VSIX_LuJing() { const external = process.env.CURSOR_LANGUAGE_PACK_VSIX; return external && fs.existsSync(external) ? external : null; }
async function QueBao_PiPei_YuYan_Bao_VSIX(): Promise<{ file: string; temporary: boolean } | null> {
  const cursor = HuoQu_Cursor_VSCode_BanBen(), local = HuoQu_Bundled_VSIX_LuJing(); if (local && YanZheng_VSIX(local) && (!cursor || YuYan_Bao_BanBen_PiPei(DuQu_VSIX_BanBen(local), cursor))) return { file: local, temporary: false };
...
function SheZhi_XianShi_YuYan() { const file = path.join(CURSOR_SHU_JU_LU_JING, "User", "locale.json"); fs.mkdirSync(path.dirname(file), { recursive: true }); if (fs.existsSync(file)) try { if (readJson(file).locale === XUAN_SHI_YU_YAN) return true; } catch {} writeJson(file, { locale: XUAN_SHI_YU_YAN }); return true; }
async function AnZhuang_GuanFang_YuYan_Bao() { const packageInfo = await QueBao_PiPei_YuYan_Bao_VSIX(); if (!packageInfo) { console.log("[语言包] 未找到可用 VSIX，跳过官方语言包安装，继续执行注入汉化..."); return false; } const cli = HuoQu_Cursor_CLI_LuJing(); if (!fs.existsSync(cli)) return false; try { const r = spawnSync(cli, ["--install-extension", packageInfo.file, "--force", `--user-data-dir=${CURSOR_SHU_JU_LU_JING}`], { encoding: "utf8", timeout: 120000, shell: Shi_Windows() }); const output = `${r.stdout || ""}${r.stderr || ""}`.trim(); if (output) for (const line of output.split("\n")) if (line.trim()) console.log(`[语言包] ${line.trim()}`); if (r.status !== 0) return false; SheZhi_XianShi_YuYan(); return true; } finally { if (packageInfo.temporary) { try { fs.rmSync(path.dirname(packageInfo.file), { recursive: true, force: true }); } catch {} } } }
function HuoQu_YuYan_Bao_PeiZhi_LuJing() { return path.join(CURSOR_SHU_JU_LU_JING, "languagepacks.json"); }
function DuQu_YuYan_Bao_PeiZhi(): [JsonObject | null, string] { const file = HuoQu_YuYan_Bao_PeiZhi_LuJing(); if (!fs.existsSync(file)) return [null, file]; try { return [readJson(file), file]; } catch { return [null, file]; } }
...
async function ZhuChengXu() {
  ...
  if (arg === "--fix-checksum" || arg === "--fix-jiaoyan" || arg === "--xiufu-jiaoyan") {
  ...
  try {
    ...
    if (!process.argv.includes("--no-restart") && arg !== "--fix-checksum" && arg !== "--fix-jiaoyan" && arg !== "--xiufu-jiaoyan") QiDong_Cursor();
  }
}
